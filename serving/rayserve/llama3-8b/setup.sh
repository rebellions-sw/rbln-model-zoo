...
echo "=================================================================="
echo "= Step 3. Llama3-8B model compile                                ="
echo "=================================================================="

LLAMA_3B_DIR="Meta-Llama-3-8B-Instruct"
MODEL_DIR="."
...
if [ ! -d ${MODEL_DIR}/${LLAMA_3B_DIR} ]; then
  ...
  if [ $? -ne 0 ] || [ ! -d ${MODEL_DIR}/${LLAMA_3B_DIR} ]; then
    echo "Error while model compile."
    popd
    exit 1
  fi
else
  echo " \"${MODEL_DIR}/${LLAMA_3B_DIR}\" already exists."
fi

echo "Done."

echo "All preparation is done."
echo "You can start with serve cli in \"output\" directory(ex. \"serve run serve:app --name llama3-8b\")."

echo "Example request:"
echo "$ curl -sN -H \"Content-Type: application/json\" -X POST http://localhost:8000/v1/completions \\"
echo "    --data-raw '{"
echo "      \"model\": \"Meta-Llama-3-8B-Instruct\","
