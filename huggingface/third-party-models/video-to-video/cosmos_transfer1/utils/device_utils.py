...
class RBLNDeviceAllocator:
    ...
    def print_allocation_summary(self):
        ...
        total_devices = (
            max(max(devices) for devices in self.allocation_result.values()) + 1
        ...
        for model in self.models:
            print(f" - {model.name}: {[f'{usage:.2f}' for usage in model.alloc_per_node]} GiB")
        print(f"{'=' * table_width}")
        ...
        sorted_devices = sorted(device_usage.keys())
        device_chunks = [
            sorted_devices[i : i + devices_per_row]
            for i in range(0, len(sorted_devices), devices_per_row)
        ]
        ...
        for chunk_idx, device_chunk in enumerate(device_chunks):
            ...
            max_models_in_chunk = max(len(device_usage[device]) for device in device_chunk)
            ...
            for device in device_chunk:
                used_memory = device_memory_usage[device]
