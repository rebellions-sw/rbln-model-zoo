...
from rbln_module.attention import Attention, GPT2FeedForward
...
class FourierFeatures(nn.Module):
    """
    Implements a layer that generates Fourier features from input tensors, based on
    randomly sampled frequencies and phases. This can help in learning high-frequency
    functions in low-dimensional problems.

    [B] -> [B, D]

    Parameters:
        num_channels (int): The number of Fourier features to generate.
        bandwidth (float, optional): The scaling factor for the frequency of the Fourier
            features. Defaults to 1.
        normalize (bool, optional): If set to True, the outputs are scaled by sqrt(2),
            usually to normalize the variance of the features. Defaults to False.
    ...
    def forward(self, x, gain: float = 1.0):
        ...
        Args:
            ...
            gain (float, optional): An additional gain factor applied during the forward
                pass. Defaults to 1.
...
class PatchEmbed(nn.Module):
    """
    PatchEmbed is a module for embedding patches from an input tensor by applying
    either 3D or 2D convolutional layers, depending on the input format. This module
    can process inputs with temporal (video) and spatial (image) dimensions, making it
    suitable for video and image processing tasks. It supports dividing the input into
    patches and embedding each patch into a vector of size `out_channels`.
    ...
    - bias (bool): If True, adds a learnable bias to the output of the convolutional
        layers. Default: True.
    """
...
class VideoAttn(nn.Module):
    ...
    This module processes video features while maintaining their spatio-temporal
    structure. It can perform self-attention within the video features or
    cross-attention with external context features.
    ...
    Parameters:
        ...
        qkv_norm_mode (str): Normalization mode for query/key/value projections.
            Must be "per_head". Default: "per_head"
        x_format (str): Format of input tensor. Must be "BTHWD". Default: "THWBD"
        n_views (int): Extra parameter used in multi-view diffusion model. It indicated
            total number of camera we model together.
    Input shape:
    ...
    def forward(
        ...
        Args:
            x (Tensor): Input tensor of shape (B, T, H, W, D) or (T, H, W, B, D)
                representing batches of video data.
            ...
            Rotary positional embedding tensor of shape (L, 1, 1, D).
                L == THW for current video training.
...
class DITBuildingBlock(nn.Module):
    """
    A building block for the DiT (Diffusion Transformer) architecture that supports
    different types of attention and MLP operations with adaptive layer normalization.
    ...
    def __init__(
        ...
        if block_type in ["cross_attn", "ca"]:
            ...
            self.block = GPT2FeedForward(
                x_dim, int(x_dim * mlp_ratio), dropout=mlp_dropout, bias=bias
            )
        else:
    ...
    def forward(
        ...
        Args:
            x (Tensor): Input tensor of shape (B, T, H, W, D) or (T, H, W, B, D).
                Current only support (T, H, W, B, D).
            ...
            rope_emb_L_1_1_D (Optional[Tensor]): Rotary positional embedding tensor of
                shape (L, 1, 1, D). L == THW for current video training.
        ...
        Returns:
            Tensor: The output tensor after processing through the configured block and
...
class GeneralDITTransformerBlock(nn.Module):
    """
    A wrapper module that manages a sequence of DITBuildingBlocks to form a complete
    transformer layer.
