...
class NewsCategoryClassification(T5EncoderModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels

        print("Single task")
        # self.pooler = SimplePoolerWithClassifier(config, self.num_labels) #Edited by sbh
        self.pooler = models.Pooling(2048, pooling_mode="mean")  # sbh added
    ...
    def forward(
        ...
        # logits = self.pooler(last_hidden_state, attention_mask) #Edit by Sbh
        feature = {
            "token_embeddings": last_hidden_state,
            "attention_mask": attention_mask,
        }  # sbs added
        logits = self.pooler(feature)  # sbs added
        print("logits", logits)
...
class KTULMEncoderForSequenceClassificationSimple(T5EncoderModel):
    ...
    def forward(
        ...
        if self.config.problem_type == "single_label_classification":
            print("logits", logits)
            print("token embedding shape", logits["token_embeddings"].shape)
            print("sentence_embedding shape", logits["sentence_embedding"].shape)
